...
что и исходный, но вот сделать append уже не получится (для передаваемого слайса), так как капасити в данном случае будет увеличен, следовательно на базовый слайс это никак не повлияет.
```
```shell
Что выведет данная программа и почему?
...
Вывод:
[b b a] Вывод слайса анонимной фукнции.
[a a] Так как мы функцией append принудительно выделили новое место в памяти и далее поменяли значения в уже новом слайсе, который находится в анонимной функции.
```
